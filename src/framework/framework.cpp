...
void Framework::debug_gui() {
    ...
    if( ImGui::InputText("Command line", command, IM_ARRAYSIZE(command), ImGuiInputTextFlags_EnterReturnsTrue) ) {
    std::vector<std::string> command_parsed = parse_command(command);
    run_command(command_parsed);
    }
